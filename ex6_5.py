...
def your_function(datapath):
    ...
    for i in data:
        try:
            if i['html_url'] != '':
                result.append({'login': i['login'], 'html_url': i['html_url'], 'contributions': i['contributions']})
            else:
                result.append({'login': i['login'], 'html_url': 'https://github.com/' + i['login'], 'contributions': i['contributions']})
        except KeyError:
            result.append({'login': i['login'], 'html_url': 'https://github.com/' + i['login'], 'contributions': i['contributions']})
    return result
